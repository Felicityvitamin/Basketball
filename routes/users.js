...
var indexPost = function(req, res) {
    ...
    conn.query('SELECT password from users WHERE name=?', [req.body.uname], function(err, result) {
        ...
        else if (result[0].password != req.body.psw) {
            res.redirect('/')
            return;
        } else {
...
var createPost = function(req, res) {
    ...
    conn.query('INSERT INTO board (name, roomname, area, roomstate,time,password) VALUES (?,?,?,?,?,?)', [ // 데이터베이스에 삽입
        req.session.user.id, body.roomname, body.area, body.roomstate, body.time, body.password
    ], function() {
...
var editorPost = function(req, res) {
    var body = req.body;
    conn.query('UPDATE board SET roomname=?,area=?, roomstate=?, password=?, time=? WHERE count=?', [body.roomname, body.area, body.roomstate, body.password, body.time, req.params.count],
        function() {
            res.redirect('/board')
        });
}
